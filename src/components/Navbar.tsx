...
export default function Navbar() {
  const { cartCount, wishlistCount, compareList, user: appUser, wishlist, viewedProducts, logout: appLogout } = useApp()
  const { isLoggedIn: authLoggedIn, isAdmin: authIsAdmin, logout: authLogout, loading: authLoading, user: authUser } = useAuth()
  ...
  // Combinar estados de ambos contextos
  const isLoggedIn = authLoggedIn || !!appUser
  const user = appUser || authUser
  // Verificar si es admin - directamente por email o por rol
  const isAdmin = authIsAdmin || user?.isAdmin === true || user?.email === 'juandavidgudelonoa@gmail.com'
  const logout = authLogout || appLogout
    ...
    <header className="bg-white/95 backdrop-blur-md shadow-sm sticky top-0 z-50 border-b border-gray-100">
      <div className="container mx-auto px-4 py-3 flex justify-between items-center flex-wrap gap-4">
        ...
        <nav className="hidden lg:flex items-center gap-1">
          <Link to="/" className={`nav-item px-4 py-2 rounded-lg font-medium transition-all duration-200 hover:bg-gray-100 ${isActive('/') ? 'bg-gradient-to-r from-amber-500/10 to-teal-500/10 text-amber-600' : 'text-gray-700 hover:text-amber-600'}`}>
            Inicio
          </Link>
          ...
          <div className="relative">
            <button 
              ...
              className={`nav-item px-4 py-2 rounded-lg font-medium transition-all duration-200 flex items-center gap-1 hover:bg-gray-100 ${isActive('/catalog') ? 'bg-gradient-to-r from-amber-500/10 to-teal-500/10 text-amber-600' : 'text-gray-700 hover:text-amber-600'}`}
            >
              Catálogo
              <ChevronDown className={`w-4 h-4 transition-transform duration-300 ${catalogOpen ? 'rotate-180' : ''}`} />
            </button>
          ...
          <Link to="/contact" className={`nav-item px-4 py-2 rounded-lg font-medium transition-all duration-200 hover:bg-gray-100 ${isActive('/contact') ? 'bg-gradient-to-r from-amber-500/10 to-teal-500/10 text-amber-600' : 'text-gray-700 hover:text-amber-600'}`}>
            Contacto
          </Link>
        </nav>

        {/* Icons - Solo mostrar cuando hay sesión iniciada */}
        <div className="flex items-center gap-1 md:gap-2">
          {isLoggedIn ? (
            <>
              ...
              {/* Compare */}
              <Link to="/compare" className="hidden sm:block relative p-2 hover:text-teal-600 transition btn-bounce" title="Comparar">
              ...
              <div className="hidden sm:block relative">
                <button type="button" onClick={() => setHistoryOpen(!historyOpen)} className="p-2 hover:text-teal-600 transition btn-bounce" title="Historial">
              ...
              {/* Wishlist */}
              <div className="relative">
                <button type="button" onClick={() => setWishlistOpen(!wishlistOpen)} className="relative p-2 hover:text-red-500 transition btn-bounce">
              ...
              {/* Cart */}
              <Link to="/cart" className="relative p-2 hover:text-teal-600 transition btn-bounce">
            ...
            /* Solo botón Ingresar cuando NO hay sesión */
            <Link to="/login" className="px-4 py-2 bg-gradient-to-r from-orange-500 to-red-500 text-white rounded-full font-medium hover:opacity-90 transition btn-bounce">
              Ingresar
            </Link>
          )}
          ...
          {/* Mobile Menu Button */}
          <button 
            className="lg:hidden p-2 btn-bounce"
