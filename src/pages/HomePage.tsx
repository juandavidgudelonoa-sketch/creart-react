  ...
  return (
    <div>
      ...
      >
        <div className="container mx-auto px-4 text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-3 md:gap-4 justify-center px-4">
            <Link to="/catalog" className="inline-block bg-gradient-to-r from-orange-500 to-red-500 text-white py-3 px-6 md:px-8 rounded-full font-semibold hover:opacity-90 transition text-sm md:text-base">
              Explorar Colección
            </Link>
            <a href="https://wa.me/573159934696" target="_blank" className="inline-block bg-green-500 text-white py-3 px-6 md:px-8 rounded-full font-semibold hover:bg-green-600 transition text-sm md:text-base">
      ...
      <section className="py-8 md:py-16 container mx-auto px-4">
        <h2 className="text-2xl md:text-3xl font-bold text-center mb-6 md:mb-12 relative">
          Nuestras Categorías
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 md:gap-6">
          {categories.filter((cat: any) => storeSettings.categoriesVisibility?.[cat.id] !== false).map((cat: any) => (
            <Link to={`/catalog/${cat.id}`} key={cat.id} className="bg-white rounded-lg shadow-md p-4 md:p-6 text-center hover:shadow-xl transition">
              {cat.image ? (
      ...
      <section className="py-8 md:py-16 bg-gradient-to-b from-teal-600 to-green-600 text-white">
        <div className="container mx-auto px-4">
          <h2 className="text-2xl md:text-3xl font-bold text-center mb-6 md:mb-12">Opiniones de Clientes</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-8">
            {displayTestimonials.map((t: any, i: number) => (
              <div key={i} className="bg-white/10 backdrop-blur-sm rounded-lg p-4 md:p-6">
                {/* Estrellas */}
                {t.rating && (
